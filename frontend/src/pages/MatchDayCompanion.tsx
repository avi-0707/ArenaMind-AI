  ...
  return (
    <PageTransition className="space-y-6 max-w-5xl mx-auto h-[calc(100vh-8rem)] flex flex-col">
      ...
      <div className="grid md:grid-cols-4 gap-6 flex-1 min-h-0">
        <nav className="md:col-span-1 flex flex-col gap-2 overflow-y-auto pr-2" aria-label="Quick actions list">
          ...
          {shortcuts.map((shortcut, i) => (
            <motion.button 
              ...
              className="flex items-center gap-3 px-4 py-3 bg-card border border-border rounded-lg text-sm font-medium hover:bg-muted hover:border-primary transition-colors text-left shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-primary"
              aria-label={`Ask: ${shortcut.label}`}
            >
              <div className="text-primary" aria-hidden="true">{shortcut.icon}</div>
              {shortcut.label}
            </motion.button>
          ))}
        </nav>
          ...
          >
            <AnimatePresence initial={false}>
              {messages.map((msg, i) => (
                ...
                >
                  {msg.role !== 'user' && (
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'error' ? 'bg-orange-500/20' : 'bg-primary/20'}`} aria-hidden="true">
          ...
          <div className="p-4 border-t border-border bg-card">
            ...
            >
              <input
                ...
                className="flex-1 px-4 py-2.5 bg-background border border-border rounded-full text-sm focus:outline-none focus:border-primary transition-colors focus:ring-2 focus:ring-primary focus:ring-offset-1"
                disabled={isLoading}
              />
              <motion.button
                ...
                className="w-10 h-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center disabled:opacity-50 hover:bg-primary/90 transition-colors shrink-0 shadow-sm focus:outline-none focus:ring-2 focus:ring-primary"
              >
