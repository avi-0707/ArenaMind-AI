  ...
  return (
    <PageTransition className="space-y-6 max-w-5xl mx-auto h-[calc(100vh-8rem)] flex flex-col">
      ...
      <div className="grid md:grid-cols-4 gap-6 flex-1 min-h-0">
        <nav className="md:col-span-1 flex flex-col gap-2 overflow-y-auto pr-2" aria-label="Staff quick actions list">
          ...
          {shortcuts.map((shortcut, i) => (
            <motion.button 
              ...
              className="flex items-center gap-3 px-4 py-3 bg-card border border-border rounded-lg text-sm font-medium hover:bg-muted hover:border-blue-500 transition-colors text-left shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-blue-500"
              aria-label={`Ask: ${shortcut.label}`}
            >
              <div className="text-blue-500" aria-hidden="true">{shortcut.icon}</div>
              {shortcut.label}
            </motion.button>
          ))}
        </nav>
          ...
          >
            <AnimatePresence initial={false}>
              {messages.map((msg, i) => (
                ...
                >
                  {msg.role !== 'user' && (
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'error' ? 'bg-orange-500/20' : 'bg-blue-500/20'}`} aria-hidden="true">
                      {msg.role === 'error' ? <AlertCircle className="w-5 h-5 text-orange-500" /> : <Bot className="w-5 h-5 text-blue-500" />}
                    </div>
                  )}
                  
                  {msg.role === 'error' ? (
                    <div className="px-4 py-3 rounded-2xl max-w-[80%] text-sm shadow-sm bg-orange-500/10 border border-orange-500/20 text-orange-700 dark:text-orange-300 rounded-tl-sm" role="alert">
                       <h3 className="font-bold flex items-center gap-2 mb-1">
                          <AlertCircle className="w-4 h-4" aria-hidden="true" /> Gemini temporarily unavailable
                        </h3>
                        <p className="flex items-center gap-2 mb-2 text-xs opacity-80">
                          <Activity className="w-3 h-3 animate-spin-slow" aria-hidden="true" /> Switching to ArenaMind Rule Engine...
                        </p>
                        <p className="text-xs">
            ...
            {isLoading && (
              ...
              >
                <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center shrink-0" aria-hidden="true">
          ...
          <div className="p-4 border-t border-border bg-card">
            ...
            >
              <input
                ...
                aria-label="Staff query input"
                className="flex-1 px-4 py-2.5 bg-background border border-border rounded-full text-sm focus:outline-none focus:border-blue-500 transition-colors focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"
                disabled={isLoading}
              />
              <motion.button
                ...
                aria-label="Send message to staff assistant"
                className="w-10 h-10 rounded-full bg-blue-500 text-white flex items-center justify-center disabled:opacity-50 hover:bg-blue-600 transition-colors shrink-0 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
